module.exports = {
    ...
    userProfile: async (userId, currentUser) => {
        const userProfile = await db.models.user.aggregate([
            ...
            { $unwind: { path: "$isFollowed", preserveNullAndEmptyArrays: true } },
            {   //TODO following status is true
                $project: {
                    allPosts: "$TotalPosts", isFollowed: 1, posts: { $size: "$TotalPosts" }, name: 1, eamil: 1, path: 1, createdOn: 1, account_privacy: 1
                    , isVisible: {
                        $or: [{ $eq: ["$account_privacy", "public"] }, { $eq: ["$isFollowed.status", "following"] }]
                    }
                }
            },
        ]);
